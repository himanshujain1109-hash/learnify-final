...
export const TeacherAvatar: React.FC<TeacherAvatarProps> = ({
  ...
  if (mode === 'bubble') {
    return (
      <div className="flex items-center gap-3 bg-white/95 backdrop-blur-md border border-slate-200 rounded-full px-3.5 py-1.5 shadow-md">
        <div className="relative w-10 h-10 rounded-full overflow-hidden border-2 border-indigo-500 bg-slate-100">
        ...
        <div className="text-left">
          <p className="text-xs font-bold text-slate-800 leading-none">{teacher.name}</p>
          <p className="text-[10px] text-indigo-600 font-semibold mt-0.5">Live Instructor</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col items-center select-none w-full max-w-[240px]">
      ...
      <div className="flex items-center gap-2 mb-2 px-3 py-1 bg-white border border-slate-200 rounded-full shadow-2xs">
        ...
        {isSpeaking && (
          <div className="flex items-center gap-0.5 ml-1">
            ...
            <span className="w-0.5 h-3.5 bg-emerald-500 animate-pulse delay-75" />
            <span className="w-0.5 h-2.5 bg-emerald-500 animate-pulse delay-150" />
          </div>
        )}
      </div>
      ...
      <div className="relative w-44 h-56 flex items-end justify-center">
        {/* Subtle light glow behind teacher */}
        <div className="absolute inset-0 bg-indigo-50/50 blur-xl rounded-full pointer-events-none" />

        <svg
          viewBox="0 0 200 240"
          className="w-full h-full filter drop-shadow-[0_8px_16px_rgba(0,0,0,0.08)]"
        >
          <defs>
      ...
      {/* Teacher Nameplate */}
      <div className="w-full text-center mt-1.5 px-2.5 py-1.5 bg-white border border-slate-200 rounded-xl shadow-xs">
        <p className="text-xs font-bold text-slate-900 truncate">{teacher.name}</p>
        <p className="text-[10px] text-indigo-600 font-semibold truncate">{teacher.title}</p>
      </div>
    </div>
  );
};
