...
export const ClassroomBoard: React.FC<ClassroomBoardProps> = ({
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 sm:gap-6 flex-1 items-center my-2 overflow-y-auto pr-1">
        {/* Bullets Column */}
        <div className={`${slide.visualDiagram?.elements?.length ? 'lg:col-span-7' : 'lg:col-span-12'} space-y-3`}>
          {(slide.bullets || []).map((bullet, idx) => {
            ...
            return (
              <motion.div
                ...
                className={`relative p-3.5 sm:p-4 rounded-xl border transition-all duration-300 ${
                  isHighlighted
                    ? 'bg-indigo-50/90 border-indigo-400 ring-2 ring-indigo-200 text-indigo-950 shadow-sm'
        ...
        {/* Right Column: 2D Visual Infographic Diagram (5 cols) only if elements exist */}
        {Boolean(slide.visualDiagram?.elements?.length) && (
