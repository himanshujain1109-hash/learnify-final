...
async function callGeminiWithFallback(ai: GoogleGenAI, requestConfig: any) {
  // Use robust, supported candidate models with fallback cascade
  ...
  for (const model of candidateModels) {
    ...
    while (attempts < maxAttemptsForModel) {
      attempts++;
      try {
        console.log(`[Gemini API] Requesting generation with model: ${model} (attempt ${attempts})`);
        ...
        console.log(`[Gemini API] Model ${model} returned error status: ${errStatus || 'unknown'}, message: ${errMsg.slice(0, 120)}`);

        if (isTransientUnavailable && attempts < maxAttemptsForModel) {
          console.log(`[Gemini API] Model ${model} is experiencing high demand (503). Retrying after 1000ms...`);
          await new Promise((resolve) => setTimeout(resolve, 1000));
        } else {
          // Cascade to the next model in candidateModels
          break;
        }
      }
    }
...
app.post('/api/generate-lecture', async (req: Request, res: Response) => {
  try {
    ...
    const targetSlideCount = slideCount || Math.max(3, Math.min(9, Math.round(effectiveDurationMinutes * 0.65)));
...
4. Provide an overall lecture title, subject, executive overviewSummary, keyTermsGlossary (4-6 terms), and 3 suggestedReviewQuestions for students.`;
    ...
    try {
      ...
      if (textOutput) {
        lectureData = JSON.parse(textOutput);
      }
    } catch (modelErr: any) {
